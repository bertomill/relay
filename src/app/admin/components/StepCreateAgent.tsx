...
export default function StepCreateAgent({ onComplete, isComplete }: StepCreateAgentProps) {
  ...
  const [deployResult, setDeployResult] = useState<{ name?: string; url?: string; error?: string } | null>(null);
  ...
  // Deploy agent from current document
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (!user) {
        ...
        setDeployResult({ error: "You must be signed in to deploy" });
        return;
      }
      const res = await fetch("/api/agents/deploy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ specMarkdown: documentContent, userId: user.id }),
      });
      ...
      setDeployResult({ name: data.name, url: data.url });
    } catch (err) {
      setDeployStatus("error");
      ...
      {showChat && (
        <div className="fixed inset-0 z-50 bg-[#FAFAF8] flex flex-col">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[#E8E6E1] bg-white shrink-0">
            ...
            {/* Deploy button — only visible when document has content */}
            {documentContent.trim() && (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                    deployStatus === "deploying"
                      ? "bg-[#6B8F71]/50 text-white cursor-wait"
                ...
                >
                  ...
                  {deployStatus === "deploying" ? "Deploying..." : deployStatus === "success" ? "Deployed!" : "Deploy Agent"}
                </button>
              </div>
            )}
